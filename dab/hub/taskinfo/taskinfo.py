...
from dab.db import taskinfo

import pdb

class TaskListHandler(HandlerBase):
    def prepare(self):
        if self.db_pool_hander:
            self.db_pool_hander.connect()
        pass
    def on_finish(self):
        if not self.db_pool_hander.is_closed():
            self.db_pool_hander.close()
        super(TaskListHandler, self).on_finish()
...
class TaskTopHandler(HandlerBase):
    def get(self,count):
        self.http_buffer_loading()
        if not (hasattr(self,"channelname") and hasattr(self,"hostname")) :
            self.write(self.ret_404_msg("404 error"))
        ...
        k = ['taskid','state','createtime',
             'buildname','srcname','srcversion',
             'srcdsc_file']
        task_data = taskinfo.get_top_daemoncli(self.session,self.channelname,self.hostname,count)
        ret_task_data = []
        for v in task_data:
            ret_task_data.append(dict(zip(k,v)))
        ret_data = {'retcode':0,'retmsg':None,'list':ret_task_data}
